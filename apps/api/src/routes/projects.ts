...
projectRoutes.get("/:teamId/projects/:id", zValidator("param", ProjectId, validatorHook), async (c) => {
	const userId = c.get("userId");
	const { teamId } = c.req.param();
...
projectRoutes.post("/:teamId/projects", zValidator("json", ProjectInsert, validatorHook), async (c) => {
	const userId = c.get("userId");
	const { teamId } = c.req.param();
	...
	const [project] = await db.insert(projects).values({ ...body, teamId }).returning();
	if (!project) {
...
projectRoutes.patch("/:teamId/projects/:id", zValidator("json", ProjectUpdate, validatorHook), async (c) => {
	const userId = c.get("userId");
...
projectRoutes.delete("/:teamId/projects/:id", zValidator("param", ProjectId, validatorHook), async (c) => {
	const userId = c.get("userId");
	const { teamId } = c.req.param();
	const { id } = c.req.valid("param");
	const [existing] = await db
