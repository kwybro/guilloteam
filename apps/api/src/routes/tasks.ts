...
taskRoutes.get(
	"/:teamId/projects/:projectId/tasks/:id",
	zValidator("param", TaskId, validatorHook),
	async (c) => {
		const userId = c.get("userId");
		const { projectId } = c.req.param();
...
taskRoutes.post(
	...
	zValidator("json", TaskInsert, validatorHook),
	async (c) => {
		const userId = c.get("userId");
		const { projectId } = c.req.param();
...
taskRoutes.patch(
	"/:teamId/projects/:projectId/tasks/:id",
	zValidator("json", TaskUpdate, validatorHook),
	async (c) => {
		const userId = c.get("userId");
